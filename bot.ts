...
const DISCLAIMER =
  ...
  `Бот <b>«Свои в Братиславе»</b> — это информационная платформа сообщества. Администрация бота:\n\n` +
  `• не проверяет и не верифицирует публикуемые данные\n` +
  `• не несёт ответственности за достоверность информации о специалистах, товарах и услугах\n` +
  `• не является стороной сделок между пользователями и не несёт ответственности за их результат\n` +
  `• не гарантирует качество услуг и товаров третьих лиц\n` +
  `• не несёт ответственности за действия или бездействие лиц, упомянутых в боте\n\n` +
  `Вся информация публикуется пользователями самостоятельно и отражает их личное мнение.\n\n` +
  `Используя бот, вы подтверждаете что ознакомлены с этими условиями, действуете на свой страх и риск и самостоятельно проверяете информацию перед принятием решений.\n\n` +
  `По вопросам модерации: @svoiba_admin`;
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(DISCLAIMER, { parse_mode: "HTML" });
...
bot.command("terms", (ctx) => ctx.reply(DISCLAIMER, { parse_mode: "HTML" }));
